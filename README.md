...
- `IAdtResult<T>` (since 28.0.0) - the result half, and a contract like the error half: `value` is what the member promised, `response` is what a fuller strategy adds. A result strategy chooses how much to fill in, the same way an error strategy does — both halves of an answer are contracts, or neither is
