...
- `IAdtResponse<TResult>` (`adt/IAdtResponse.ts`, since 28.0.0) - what a **member** answers with, and a **discriminated union**: `IAdtSuccess<TResult>` (`ok: true`, `getResult(): TResult`) or `IAdtFailure` (`ok: false`, `getError(): IAdtError`). Checking `answer.ok` narrows, so neither method is `| undefined` once you have asked — two independently-optional methods would let an implementation answer both or neither, and would narrow nothing. `TResult` has no default, so a member cannot answer `IAdtResponse` and promise nothing
