...
- **Profiler**: `IProfiler` — what a run left behind, not how it is configured. `list()` for what traces exist, `read(traceId, view)` for what is inside one, where `view` is `'hitlist'`, `'statements'` or `'dbAccesses'` and the result is that view's own type; `readWith(parse, traceId, view)` (since 23.0.0) and `listWith(parse, options?)` (since 25.0.0) for a consumer's own reader over a view or over the listing, each keeping its own return type; `delete(traceId)` (since 25.0.0) for taking one back out, since a profiled run was otherwise permanent. Reading-only from 22.0.0 until 25.0.0. Everything about *configuring* a measurement moved to `ITraceScheduling` above. Built from `ITraceEntry` / `ITraceView` / `ITraceListing` / `ITraceReading` / `ITraceReadingWithParser` / `ITraceListingWithParser` / `ITraceDeletion` / `ITraceFamily` (`runtime/ITrace.ts`), which any trace family composes — an implementer of `IProfiler` owes every one of those members
