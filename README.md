...
- `IAdtResponse<T>` (`adt/IAdtResponse.ts`, since 28.0.0) - what a **member** answers with, and a **discriminated union**: `IAdtSuccess<TResult>` (`ok: true`, `getResult(): TResult`) or `IAdtFailure<TError>` (`ok: false`, `getError(): TError`), with `TResult extends IAdtResult<unknown>` and `TError extends IAdtError = IAdtError`. Both halves are parameters **constrained to their contract**, so an implementation answering `IAdtError & { retryAfter: number }` can say so in the type while a caller written against `IAdtError` reads it unchanged — and `IAdtResponse<IAdtResult<string>, string>` does not compile. Checking `answer.ok` narrows, so neither method is `| undefined` once you have asked — two independently-optional methods would let an implementation answer both or neither, and would narrow nothing. The type parameter has no default, so a member cannot answer `IAdtResponse` and promise nothing
