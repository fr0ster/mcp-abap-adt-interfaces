...
- `IAdtResponse<TResult>` (`adt/IAdtResponse.ts`, since 28.0.0) - what a **member** answers with: `getResult(): TResult | undefined` and `getError(): IAdtError | undefined`. A result can be a normal answer or a failure, and the contract says so rather than naming only the first and pushing the second where the compiler cannot see it. `TResult` has no default, so a member cannot answer `IAdtResponse` and promise nothing
