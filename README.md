...
- `IAdtResponse<T>` (`adt/IAdtResponse.ts`, since 28.0.0) - what a **member** answers with, and a **discriminated union**: `IAdtSuccess<T>` (`ok: true`, `getResult(): IAdtResult<T>`) or `IAdtFailure` (`ok: false`, `getError(): IAdtError`). Checking `answer.ok` narrows, so neither method is `| undefined` once you have asked — two independently-optional methods would let an implementation answer both or neither, and would narrow nothing. The type parameter has no default, so a member cannot answer `IAdtResponse` and promise nothing
- `IAdtResult<T>` (since 28.0.0) - the result half, and a contract like the error half: `value` is what the member promised. A result strategy chooses how much to fill in, the same way an error strategy does — but how much of `T`, not how much of the wire: `brief` fills fewer fields of `ISearchResult`, it does not attach the response the hits came in. `IAdtError` keeps a `response` because diagnosing a failure needs the status it arrived with; reading a result does not
