...
- **Profiler**: the atoms, not a composition. `ITraceFamily`, `ITraceListing`, `ITraceReading`, `ITraceDeletion` and the view machinery (`ITraceView`, `ViewArgs`, `ViewResult`) are here; a profiler is their intersection with the readings an implementation performs, and `IProfiler` — which named `IAbapTraceEntry` and `IAbapTraceViews` — left in 31.0.0 along with them. What stayed on the request side is what a caller passes: `IProfilerListOptions`, `IProfilerTraceParameters` and the three per-view option types. Everything about *configuring* a measurement is `ITraceScheduling`; `read(traceId, view)` answers the view's own type, and which type that is, is the implementation's.
