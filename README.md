...
- **Cross-cutting shared types** (`adt/IAdtShared.ts`) — `AdtObjectType`(+lower/source variants), `IObjectReference`, search (`ISearchObjectsParams`/`ISearchResult`), where-used (`IGetWhereUsed*Params`/`IWhereUsedListResult`), package contents (`IPackageContentItem`/`IPackageHierarchyNode`/…, the shapes the shipped readings return), virtual folders, SQL/table-contents/discovery params, `IInactiveObjectsResponse`. (`IReadOptions` lives in `shared/IReadOptions.ts`.)
...
- **Profiler**: `IProfiler` — what a run left behind, not how it is configured. `list()` for what traces exist, `read(traceId, view)` for what is inside one, where `view` is `'hitlist'`, `'statements'` or `'dbAccesses'` and the result is that view's own type — a consumer who needs it read differently gives their implementation an `IResultStrategy` and keeps a type, which is what `readWith(parse, …)` did per call until 30.0.0 removed it; `delete(traceId)` (since 25.0.0) for taking one back out, since a profiled run was otherwise permanent. Reading-only from 22.0.0 until 25.0.0. Everything about *configuring* a measurement moved to `ITraceScheduling` above. Built from `ITraceEntry` / `ITraceView` / `ITraceListing` / `ITraceReading` / `ITraceDeletion` / `ITraceFamily` (`runtime/ITrace.ts`), which any trace family composes — an implementer of `IProfiler` owes every one of those members. `ITraceFamily` carries the name alone since 30.0.0: it held the listing too, so a family that only lists and one that also reads could not be told apart
