...
3. ~~**One credential object for HTTP and databases.**~~ **Answered.** `IAuthProvider` is HTTP-shaped (header, cookies, TLS material) and does not fit a database wire protocol, so the two stay separate contracts. Whether `connection` ever rebuilds `BasicAuthProvider`/`TokenAuthProvider` on `ISecretLoginCredential`/`IBearerCredential` is its own decision and blocks nothing here: the contracts are a vocabulary, and each acceptor writes the implementation it needs — including a consumer whose credential source neither package anticipated.
4. ~~**SAP AI SDK.**~~ **Answered — it does take a token source.** A constructed destination carries `headers.Authorization`, rebuilt per call, which is why `IBearerCredential.token()` is a function; `sap-aicore-llm` already builds its client per call. The `foundation-models` embedder is the easier case: it has its own `TokenProvider` doing client credentials over `fetch`, which `IBearerCredential` replaces outright. One gap remains for whoever implements it: the orchestration embedder constructs its client with two arguments and never threads a destination, so that seam needs wiring. See §9.2 of the llm-agent auth-contracts spec.
