...
- **Public helper/config types promoted so the full adt-clients public surface can be re-exported from here:** behaviorDefinition `IValidationResult`/`ILockResult`/`CheckReporter`/`ICheckMessage`/`ICheckRunResult`, `IEnhancementMetadata`, CDS unit-test `ICdsUnitTestConfig`/`ICdsUnitTestState`, and class-includes `ILocalTestClassConfig`/`ILocalTypesConfig`/`ILocalDefinitionsConfig`/`ILocalMacrosConfig`. (`IAdtClientOptions` stays adt-clients-local — it is the client class's own constructor options, not a shared object-type contract.)
