...
**A SAML assertion is now something a consumer can refuse to trust
unverified.** Nothing existing changes; the contract adds the surface a SAML
provider needs to stop accepting an assertion at face value.
...
- **`AssertionContext`** — what a provider knows about the login an assertion
  is answering: the expected `InResponseTo`, the audience, the ACS URL, and
  an optional trusted issuer for a validator that establishes trust some
  other way.
- **`ValidatedAssertion`** — what a validated assertion yields: an expiry
  that is never later than either the assertion's own `Conditions` or the
  bearer confirmation that was accepted, identity fields, and both the raw
  wire response and the signed XML that verified — kept distinct because
  holding the former does not make every byte of it trustworthy.
- **`IAssertionReplayStore`** and **`AssertionReplayKey`** — remembers
  assertions so a replay is refused, keyed by issuer and assertion ID
  together, since an assertion ID is unique only within the identity
...
- **BREAKING: sixty-three exported symbols, all of them result shapes or their
  ...
  **`IProfiler` stayed**, and takes its readings as parameters:
  `IProfiler<TEntry, TViews>`, no defaults, exactly as `IClassExecutor` and
  `ICrossTrace` take theirs. A published composition is a contract — a consumer
  needs it to type a profiler and to implement one, and making each of them spell
  `ITraceFamily & ITraceListing & ITraceReading & ITraceDeletion` by hand is the
  bloat decision 24 argues against, not the bloat it is about. What left are the
  shapes it used to name.
...
- **This package is now `LGPL-3.0-only`.** It was MIT up to and including 28.0.0, and
  ...
  already using 28.0.0 under MIT keeps that grant for 28.0.0.
